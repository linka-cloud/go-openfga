...
func (p *pdb) ReadPage(ctx context.Context, store string, tupleKey *openfgav1.TupleKey, options storage.ReadPageOptions) ([]*openfgav1.Tuple, []byte, error) {
	return withTx3(ctx, p, func(ctx context.Context, tx *tx) ([]*openfgav1.Tuple, []byte, error) {
		return tx.ReadPage(ctx, store, tupleKey, options)
	}, protodb.WithReadOnly())
}
...
func (p *pdb) ReadAuthorizationModels(ctx context.Context, store string, options storage.ReadAuthorizationModelsOptions) ([]*openfgav1.AuthorizationModel, []byte, error) {
	return withTx3(ctx, p, func(ctx context.Context, tx *tx) ([]*openfgav1.AuthorizationModel, []byte, error) {
		return tx.ReadAuthorizationModels(ctx, store, options)
	}, protodb.WithReadOnly())
}
...
func (p *pdb) ListStores(ctx context.Context, options storage.ListStoresOptions) ([]*openfgav1.Store, []byte, error) {
	return withTx3(ctx, p, func(ctx context.Context, tx *tx) ([]*openfgav1.Store, []byte, error) {
		return tx.ListStores(ctx, options)
	}, protodb.WithReadOnly())
}
...
func (p *pdb) ReadChanges(ctx context.Context, store, objectType string, options storage.ReadChangesOptions, horizonOffset time.Duration) ([]*openfgav1.TupleChange, []byte, error) {
	return withTx3(ctx, p, func(ctx context.Context, tx *tx) ([]*openfgav1.TupleChange, []byte, error) {
		return tx.ReadChanges(ctx, store, objectType, options, horizonOffset)
	}, protodb.WithReadOnly())
}
...
func (t *tx) ReadPage(ctx context.Context, store string, tupleKey *openfgav1.TupleKey, opts storage.ReadPageOptions) ([]*openfgav1.Tuple, []byte, error) {
	t.mu.Lock()
	defer t.mu.Unlock()
	var typ string
	if tupleKey != nil {
...
func (t *tx) ReadAuthorizationModels(ctx context.Context, store string, opts storage.ReadAuthorizationModelsOptions) (out []*openfgav1.AuthorizationModel, continuation []byte, err error) {
	t.mu.Lock()
	defer t.mu.Unlock()
...
func (t *tx) FindLatestAuthorizationModel(ctx context.Context, store string) (*openfgav1.AuthorizationModel, error) {
	t.mu.Lock()
	defer t.mu.Unlock()
	ms, _, err := typed.NewTx[pbv1.Model](t.tx).Get(ctx, &pbv1.Model{}, protodb.WithFilter(protodb.Where("key").StringHasPrefix(pbv1.ModelPrefix(store))), protodb.WithReverse())
	if err != nil {
		return nil, err
	}
	if len(ms) == 0 {
...
func (t *tx) ListStores(ctx context.Context, opts storage.ListStoresOptions) ([]*openfgav1.Store, []byte, error) {
	t.mu.Lock()
	defer t.mu.Unlock()
	...
	ss, info, err := typed.NewTx[openfgav1.Store](t.tx).Get(ctx, &openfgav1.Store{}, protodb.WithPaging(&protodb.Paging{
		Offset: tk.Offset,
...
func (t *tx) ReadChanges(ctx context.Context, store, objectType string, opts storage.ReadChangesOptions, horizonOffset time.Duration) ([]*openfgav1.TupleChange, []byte, error) {
	t.mu.Lock()
	defer t.mu.Unlock()
	...
	cs, info, err := typed.NewTx[pbv1.Change](t.tx).Get(ctx, &pbv1.Change{}, protodb.WithFilter(f), protodb.WithPaging(&protodb.Paging{
		Offset: tk.Offset,
		Limit:  uint64(opts.Pagination.PageSize),
		Token:  tk.Continuation,
	}))
