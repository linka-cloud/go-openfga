...
func (p *pdb) Read(ctx context.Context, store string, tupleKey *openfgav1.TupleKey) (storage.TupleIterator, error) {
...
func (p *pdb) ReadPage(ctx context.Context, store string, tupleKey *openfgav1.TupleKey, opts storage.PaginationOptions) ([]*openfgav1.Tuple, []byte, error) {
	offset, token := offsetAndToken(opts.From)
	o := []protodb.GetOption{protodb.WithPaging(&protodb.Paging{Offset: offset, Limit: uint64(opts.PageSize), Token: token})}
	if tupleKey != nil {
	...
	return toTuple(ts), tokenWithOffset(info.Token, offset+uint64(len(ts))), nil
}

func (p *pdb) ReadUserTuple(ctx context.Context, store string, tupleKey *openfgav1.TupleKey) (*openfgav1.Tuple, error) {
...
func (p *pdb) ReadUsersetTuples(ctx context.Context, store string, filter storage.ReadUsersetTuplesFilter) (storage.TupleIterator, error) {
...
func (p *pdb) ReadStartingWithUser(ctx context.Context, store string, filter storage.ReadStartingWithUserFilter) (storage.TupleIterator, error) {
	var f filters.Builder
	for _, v := range filter.UserFilter {
		pf := protodb.Where("key").StringHasPrefix(fmt.Sprintf("%s/%s", store, filter.ObjectType))
...
func (p *pdb) ReadAuthorizationModels(ctx context.Context, store string, opts storage.PaginationOptions) (out []*openfgav1.AuthorizationModel, continuation []byte, err error) {
	offset, token := offsetAndToken(opts.From)
	ms, info, err := typed.NewStore[pbv1.Model](p.db).Get(ctx, &pbv1.Model{}, protodb.WithFilter(protodb.Where("key").StringHasPrefix(pbv1.ModelPrefix(store))), protodb.WithPaging(&protodb.Paging{Token: token, Offset: offset, Limit: uint64(opts.PageSize)}), protodb.WithReverse())
	if err != nil {
		return nil, nil, err
	}
	for _, v := range ms {
	...
	return out, tokenWithOffset(info.Token, offset+uint64(len(ms))), nil
...
func (p *pdb) ListStores(ctx context.Context, opts storage.PaginationOptions) ([]*openfgav1.Store, []byte, error) {
	offset, token := offsetAndToken(opts.From)
	...
	return ss, tokenWithOffset(info.Token, offset+uint64(len(ss))), nil
}
...
func (p *pdb) ReadChanges(ctx context.Context, store, objectType string, opts storage.PaginationOptions, horizonOffset time.Duration) ([]*openfgav1.TupleChange, []byte, error) {
	offset, token := offsetAndToken(opts.From)
	...
	return out, tokenWithOffset(info.Token, offset+uint64(len(out))), nil
}
...
func offsetAndToken(t string) (uint64, string) {
	if len(t) == 0 {
		return 0, ""
	}
	...
	offset, _ := strconv.ParseUint(parts[0], 10, 64)
	return offset, parts[1]
}

func tokenWithOffset(t string, offset uint64) []byte {
	return []byte(fmt.Sprintf("%d:%s", offset, t))
}
...
func makeFilter(storeID, user, relation, object string) filters.FieldFilterer {
	...
	if object != "" {
		if o, id := tuple.SplitObject(object); id != "" {
			prefix := fmt.Sprintf("%s/%s:%s", storeID, o, id)
			if relation != "" {
				prefix = fmt.Sprintf("%s#%s@", prefix, relation)
			}
			f = protodb.Where("key").StringHasPrefix(prefix)
		} else {
	...
	if user != "" {
		suffix := fmt.Sprintf("@%s", user)
		if relation != "" {
			suffix = fmt.Sprintf("#%s@%s", relation, user)
		}
